...
提供 104 會員建立個人課程、參與已開設課程的平台，除了分類、搜尋課程外，不同身份的使用者可以透過介面管理自己開設或參加過的課程，或是在課程頁面中留下評價。
...
智慧販賣機，透過 API 與後台取得商品和廣告資料、即時更新機器狀態、提供多元化的支付方式，並自動回報庫存及銷售數據。
...
RiGO 計程車管理系統，管理車隊資訊，產生數據報表與可視化圖表，以及計程車定位回報、路徑重播、與乘客 SOS 車內監視。
...
Beacon 元件管理平台，蒐集與管理室內定位的數據並產生報表，提供彈性介面讓使用者自行設定儀表板的圖表內容與數目。
...
Web of Things 物聯網元件管理與服務整合的平台，管理註冊元件、類別模型，即時更新元件狀態，並可以透過圖形化的 Blockly、Node-RED 介面來進行元件的程式、邏輯設計，或是和其他的服務串聯。
...
為了練習 Python 爬蟲而實作，定期抓取好樂迪網站歌曲資料、透過 API 或爬蟲抓取各家歌詞網歌詞，並使用 Youtube API 找尋歌曲 MV，做成歌曲資料庫；網頁提供歌名、歌手、歌詞搜尋，顯示歌曲資料與 MV。
